...
/**
 * Copy of IntelliJs-Community naturalCompare edited to compare files.
 */
class NaturalFileNameComparator {

    companion object : Comparator<File> {
        private fun isDecimalDigit(c: Char) = c in '0'..'9'

        override fun compare(lhs: File?, rhs: File?) =
            naturalCompare(lhs?.absolutePath, rhs?.absolutePath, false)
